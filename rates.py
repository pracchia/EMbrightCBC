...
if __name__=='__main__':
    ...
    print('Generating samples of MTOV, RNS and alpha_d from joint distribution')
    
    Mdmins = [1e-3,1e-2]
    chi1 = np.array([0.,0.2,0.6])
    MTOV = np.random.uniform(1.8,2.6,Nsamples*10)
    RNS = np.random.normal(12.45,0.65,Nsamples*10)
    ad = np.exp(np.random.normal(-0.11,0.47,Nsamples*10))
    p = np.random.uniform(0.,1.,Nsamples*10)
    
    bd = np.exp(np.random.normal(-0.020,0.20,Nsamples))
    ...
    while k < Nsamples:
        if p_MTOV_RNS_ad(MTOV[j],RNS[j],ad[j]) >= p[j]:
            samples_mtov_rns_ad.append([MTOV[j],RNS[j],ad[j]])
            k += 1
    
        j += 1
    
    mtov_rns_ad = np.array(samples_mtov_rns_ad).T
    ...
    for k,Mdmin in enumerate(Mdmins):
        ...
        for i in range(Nsamples):
            ...
            for j in range(len(chi1)):
                ...
                jet[i,j] = (m1[:,None]>=m2[None,:])*np.where((m1[:,None]<=MTOV[i]),(MdBNS>Mdmin)[None,:]&(MremBNS>(1.2*MTOV[i])),(MdNSBH>Mdmin)&(m2[None,:]<MTOV[i]))
